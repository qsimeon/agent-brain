...
export async function notifyInterneuronOfPulse(
  ...
  const message = [
    ...
    `   CRITICAL: Forward the signal's actual data (files, URLs, content) into payload.input_data.`,
    `   If a signal contains data.files or data.urls, copy them into input_data so the actuator has the raw material.`,
    `   Example: {"payload":{"instructions":"Create a dashboard showing this data","context":"Sensor found a PCA plot","input_data":{"files":[{"filename":"plot.png","mime_type":"image/png","content_base64":"iVBOR..."}]}}}`,
    `   Ask actuators to embed images as data URLs and render data as HTML tables/charts.`,
    ``,
